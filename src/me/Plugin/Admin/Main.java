	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {		
		if(label.equalsIgnoreCase("heal")) {
			if(!(sender instanceof Player)) {
				sender.sendMessage(ChatColor.DARK_RED + "Error while executing command : Not a Player.");
				return true;
			}
		...
		if(label.equalsIgnoreCase("suicide")) {
			...
			if(!sender.hasPermission("suicide.use")) {
				sender.sendMessage(ChatColor.DARK_RED + "Error while executing command : No permission.");
				return true;
			}
			Player p = (Player) sender;
			p.setHealth(0.0);
			p.sendMessage(ChatColor.DARK_RED + "You died successfully!");
		}
